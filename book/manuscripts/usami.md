...
ここが最初の注意点で、AI はビルド確認をしてくれない場合が多いです。ビルドして確認するように、明示的に指示を出すことをお勧めします。なお、Bevy のプロジェクトは cargo でクレートを大量にインストールする必要があるので、初回ビルドに時間がかかります。そのため、AI エージェントの実行前に、Bevy をプロジェクトに追加して初回ビルドしておくとよいでしょう。

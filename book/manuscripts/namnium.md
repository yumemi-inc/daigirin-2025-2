...
>
> また紙媒体につきZenn版から一部をオミットした形になっています。Zenn版の方もぜひ見ていただけたら幸いです！
>
> Zenn版URL: <https://zenn.dev/namn1125/articles/1a40205d9f8f9a>
> 
> タイトルは同じなので検索した方が早いかもしれません。
...
とても嬉しい機能が追加されたので、 let-else文のノリで [^linkletelse] 記事を書こうかと最初は思ったのですが、素晴らしい先達記事 [^linkletchains] がありましたので本記事では `if let` 式の**周辺**知識、という位置付けで**パターンマッチのテクニック集**を紹介したいと思います！
...
[^match]: `match` 式のデフォルトパターンは論駁不可能パターンですし、論駁不可能パターンが一つだけある `match` 式も書けます。ただ後者に関しては `let` 文で良さそうです。
...
`|` を使うことで複数パターンを連結することができます。`match` 式のアームでよく使える記法です！
...
ちなみに全パターンを網羅（exhaustive）する必要がある `match` 式のアームに使った場合、この記法を使ってもしっかりと網羅性チェックは行われるので安心です。使用頻度は高くないと思いますが、知っておいて損はないでしょう。
...
<!-- 以下 ^linkletelse2 は ^linkletelse と同じURLだが、脚注が壊れるのを防ぐためあえて別々にした -->

...
[^linkannouncing]: <https://blog.rust-lang.org/2025/06/E26/Rust-1.88.0/>
...
hooqならば、 `with_conetxt` などのメソッドを呼び出すオーバーヘッドが挟まってしまいますが、エラー型の表現をシンプルなままに留め、 `match` 式なども相変わらずスッキリさせたままオブザーバビリティを得ることができます。 `match` 式の書き味、つまりパターンマッチがhooq誕生のキッカケに関わっているのです！
